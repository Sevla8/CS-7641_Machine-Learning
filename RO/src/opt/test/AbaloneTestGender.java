...
public class AbaloneTestGender implements Runnable {
	...
	private static String outputDir = "./stats";
	private static int inputLayer = 7, outputLayer = 8, trainingIterations = 10000;
	private static Instance[] instances = initializeInstances("data/gender_x_train.csv", "data/gender_y_train.csv");
	private static Instance[] test_instances = initializeInstances("data/gender_x_test.csv", "data/gender_y_test.csv");
	...
	public static void main(String[] args) throws InterruptedException {
		...
		for(Integer[] hiddenLayerShape : hiddenLayerSizes) {
			...
			//GA
			if(computeGA) {
				for (int pop = 50; pop < 700; pop += 300) {
					for (int toMate = 50; toMate < (pop/2); toMate += 250) {
						for (int toMutate = 10; toMutate < (toMate/2); toMutate += 150) {
							network = factory.createClassificationNetwork(nnLayer);
							nnop = new NeuralNetworkOptimizationProblem(set, network, measure);
		...
		for(OA oa : oa_list) {
			AbaloneTestGender trainner = new AbaloneTestGender(oa);
			trainner.start();
		}
		for(OA oa : oa_list){
			oa.t.join();
			...
			Utils.writeOutputToFile(outputDir, "GenderTest" + outFileParticule + ".csv", oa.res);
			Utils.writeOutputToFile(outputDir, "GenderTestErrors" + outFileParticule + ".csv", sb.toString());
		}
