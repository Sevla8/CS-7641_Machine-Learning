...
def to_seaborn_dataframe(consolidate_dict, wanted_value='median', value_name='median', consolidate_z_name='argz'):
	...
	for k, v in consolidate_dict.items():
		tmp = pd.DataFrame(data=v.get([v.columns[0], v.columns[1], wanted_value]), columns=[v.columns[0], v.columns[1], value_name])
...
def read_csv(file_name, read_all=False, time_limit=GAME_TIME_STEP_LIMIT):
	...
	with open(file_name, 'r') as csvfile:
		...
		for line_number, row in enumerate(reader):
			...
			if current_timestep >= 60 or read_all:
	...
	for i, t in enumerate(csv_dict.items()):
		...
		for race, action_dict, first_time_dict, other_info in t[1]:
			...
			if race == 'Zerg':
				...
				exit(10)
			input_array = np.array(race_list + list(action_dict.values()) + list(first_time_dict.values()) + list(other_info), dtype=int)
	...
	for i, t in enumerate(csv_dict.items()):
		for race, action_dict, first_time_dict, other_info in t[1]:
			...
			if race == 'Zerg':
				...
				print("unknown race:", race)
				exit(10)
			input_list = [race_id] + list(action_dict.values()) + list(first_time_dict.values()) + list(other_info)
